...
for m in range(len(check_otzyv)):
    ...
    for p in range(len(check_review)):  # приводим к одному регистру
        check_review[p] = check_review[p].lower()
    for r in range(len(check_review)):  # приводим к начальной форме
        check_review[r] = str(morph.parse(check_review[r]))
        check_review[r] = re.findall('normal_form=\'([а-я]+)', check_review[r])[0]
        #get_closest_match_with_metric(check_review[r], X, vec, textdistance.hamming)
