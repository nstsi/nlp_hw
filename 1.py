...
for m in range(len(check_otzyv)):
    ...
    #print(check_review)
    good = intersection(check_review, good_data)
    bad = intersection(check_review, bad_reviews)
    check_review_new = list(check_otzyv[m])
    if good >= bad:
        check_review_new.append('good')
    else:
        check_review_new.append('bad')
    #print(check_review_new)
    if check_review_new[2] == 'good':
